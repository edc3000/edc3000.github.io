...
> **2026-08-12 修正**:本节最初要求空板块展示具体写作路线图(正在读的论文、第一篇
> 预计标题等),并明确反对"敬请期待"类占位文案。实现阶段用户推翻了这一设计,原话:
> 「论文解读、算法解读这些空着就行,后面有内容积累就加进去」。**以下描述的是实际
> 采用并已上线的方案,取代原路线图设计**;原方案不再是设计目标,仅存于本次修正前的
> git 历史中。

论文与算法实践上线时为 0 篇。空板块使用统一占位,不编造写作路线图或具体条目:

- 板块页(`EmptySection.astro`):渲染一枚「筹备中」徽章 + 一句简短说明——
  「这个板块还没有内容,有积累后会陆续加进来。」
- 首页板块卡片(`SectionCard.astro`):显示真实篇数,0 篇同样标注「筹备中」。

理由:没有确定条目时,诚实的占位比编造一份路线图更可信;等板块有真实产出后,占位
内容会被实际文章自然替换,不需要额外维护一份会过时的计划文本。
...
| 构建 | `astro build` | 成功;仅允许 papers/algorithms/skills 三个刻意留空 collection 各产生一条 `[glob-loader] No files found matching "**/*.md"` WARN(2026-08-12 修正,原「无警告」标准与三个空 collection 的既定设计互斥,永远无法通过) |
| 死链 | 构建后扫描内部链接 | 零死链 |
