...
**交付物:** `README.md`,记录日常增删内容与部署的操作方式,使作者数月后无需重新推导。

**Files:**
- Create: `README.md`

**Interfaces:**
- Consumes: Task 1–9 的全部产出(说明须与实际实现一致)
- Produces: 无代码产物

- [ ] **Step 1: 写 README**

创建 `README.md`。四板块的 frontmatter 示例必须与 `src/content.config.ts` 的实际 schema 逐字段对应:
...
- [ ] **Step 2: 核对 README 与实际实现一致**

逐项核对三处,不一致时以代码为准修改 README:

```bash
grep -A40 'const competitions' src/content.config.ts
grep -n 'scripts' -A8 package.json
grep -rn 'data-pagefind-body' src/
```

确认:README 中四份 frontmatter 示例的字段名与可选性,与 `content.config.ts`
一致;README 列出的 pnpm 脚本确实存在;`data-pagefind-body` 的说明与实际标记位置一致。

- [ ] **Step 3: 提交**

```bash
git add README.md
git commit -m "docs: 仓库使用说明

记录增删内容、部署、以及纯 HTML 页面的三种处理方式。
